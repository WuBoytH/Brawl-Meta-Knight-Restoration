...
#[status_script( agent = "metaknight", status = FIGHTER_STATUS_KIND_ESCAPE_B, condition = LUA_SCRIPT_STATUS_FUNC_INIT_STATUS )]
unsafe fn metaknight_escape_b_init(fighter: &mut L2CFighterCommon) -> L2CValue {
    sv_kinetic_energy!(
        clear_speed_ex,
        fighter,
        FIGHTER_KINETIC_ENERGY_ID_DAMAGE
    );
    let mut hit_xlu_frame = WorkModule::get_param_float(fighter.module_accessor, hash40("param_motion"), hash40("escape_b_hit_xlu_frame"));
    let hit_normal_frame = WorkModule::get_param_float(fighter.module_accessor, hash40("param_motion"), hash40("escape_b_hit_normal_frame"));
